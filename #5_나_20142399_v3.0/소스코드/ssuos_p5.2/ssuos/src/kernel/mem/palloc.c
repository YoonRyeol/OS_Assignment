...
palloc_get_multiple (size_t page_cnt)
{
	...
	uint32_t key = (uint32_t)(pages - VKERNEL_HEAP_START)/PAGE_SIZE;
//	현재 생성된 page의 주소를 level hash table에 삽입
	insert_elem((uint32_t*)pages, CAPACITY);
...
//	현재 해제하려는 page의 주소를 level hash table에서 제거
	delete_elem((uint32_t*)pages, CAPACITY);
	struct khpage *khpage = freelist.list;
